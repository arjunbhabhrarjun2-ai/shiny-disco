...
function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen" style={{ background: C.bg, color: C.text, fontFamily: 'var(--font-geist-stack)' }}>
      <header className="sticky top-0 z-20 px-6 h-16 flex items-center justify-between border-b" style={{ background: 'rgba(6,9,15,0.85)', backdropFilter: 'blur(12px)', borderColor: C.border }}>
        <div className="flex items-center gap-3">
          <span className="text-lg font-black tracking-tight">Admin</span>
          <span className="text-[10px] font-black uppercase tracking-[0.25em] px-2 py-1 rounded" style={{ background: 'rgba(212,175,127,0.12)', color: C.gold }}>Control Center</span>
        </div>
        <a href="/dashboard" className="text-[11px] font-bold uppercase tracking-widest" style={{ color: C.sub }}>← Exit</a>
      </header>

      <nav className="px-6 flex gap-1 border-b overflow-x-auto" style={{ borderColor: C.border }}>
        {(['overview', 'approvals', 'users', 'transactions', 'support', 'audit'] as Tab[]).map((t) => {
          const n = badges[t] || 0;
          return (
            <button key={t} onClick={() => setTab(t)} className="px-4 py-3 text-[12px] font-bold uppercase tracking-widest whitespace-nowrap flex items-center gap-2"
        ...
        <tbody>
          ...
          {rows.map((r) => (
            <tr key={r.id} className="border-t" style={{ borderColor: C.border }}>
              ...
              <td className="p-3 font-mono">{money(r.amount)}</td>
              ...
              <td className="p-3 text-[11px] font-mono" style={{ color: C.sub }}>{when(r.createdAt)}</td>
              <td className="p-3">
                <div className="flex gap-2">
                  <button disabled={busy === `${kind}-${r.id}`} onClick={() => act(kind, r.id, 'approve')}
                    className="text-[10px] font-bold uppercase px-2.5 py-1 rounded disabled:opacity-50"
    ...
    <div>
      <div className="flex items-center justify-between mb-4 gap-3">
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search name / email / username…"
          className="bg-black/40 border rounded-lg px-3 py-2 text-sm w-full max-w-md" style={{ borderColor: C.border, color: C.text }} />
...
function EditUser({ user, onClose, onSaved }: { user: AdminUser; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.6)' }} onClick={onClose}>
      <div className="rounded-xl p-6 w-full max-w-md" style={{ background: C.panel, border: `1px solid ${C.border}` }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          {([['firstName', 'First name'], ['lastName', 'Last name'], ['username', 'Username'], ['email', 'Email'], ['phone', 'Phone'], ['mainBalance', 'Main balance (USDT)']] as const).map(([k, label]) => (
            <div key={k}>
              ...
              <input value={(form as Record<string, string>)[k]} onChange={(e) => set(k, e.target.value)} className="w-full bg-black/40 border rounded-lg px-3 py-2 text-sm mt-1" style={{ borderColor: C.border, color: C.text }} />
            </div>
          ))}
          <div>
            <label className="text-[10px] uppercase font-bold tracking-widest" style={{ color: C.sub }}>Role</label>
            <select value={form.role} onChange={(e) => set('role', e.target.value)} className="w-full bg-black/40 border rounded-lg px-3 py-2 text-sm mt-1" style={{ borderColor: C.border, color: C.text }}>
        ...
        <div className="flex gap-2 mt-5">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-lg text-[11px] font-bold uppercase" style={{ background: 'rgba(255,255,255,0.06)', color: C.sub }}>Cancel</button>
          <button onClick={save} disabled={saving} className="flex-1 py-2.5 rounded-lg text-[11px] font-black uppercase" style={{ background: C.gold, color: C.bg }}>{saving ? 'Saving…' : 'Save'}</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div className="flex gap-2 mb-4 flex-wrap">
        {types.map((t) => (
          <button key={t || 'all'} onClick={() => setType(t)} className="text-[11px] font-bold uppercase px-3 py-1.5 rounded"
          ...
          <tbody>
            ...
            {rows.map((t) => (
              <tr key={t.id} className="border-t" style={{ borderColor: C.border }}>
                ...
                <td className="p-3 font-mono">{money(t.amount)}</td>
                <td className="p-3"><span className="text-[10px] font-bold uppercase" style={{ color: t.status === 'Success' ? C.green : t.status === 'Pending' ? C.gold : C.red }}>{t.status}</span></td>
                <td className="p-3 text-[11px]" style={{ color: C.sub }}>{t.description}</td>
                <td className="p-3 text-[11px] font-mono" style={{ color: C.sub }}>{when(t.createdAt)}</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <div>
      <div className="flex gap-2 mb-4">
        {(['open', 'resolved'] as const).map((k) => (
          <button key={k} onClick={() => { setTab(k); setOpenId(null); }} className="text-[11px] font-bold uppercase px-3 py-1.5 rounded"
              ...
              {openId === t.id && (
                <div className="p-4 border-t" style={{ borderColor: C.border }}>
                  ...
                  {t.status === 'resolved' ? (
                    ...
                    <div className="flex gap-2">
                      <input value={reply} onChange={(e) => setReply(e.target.value)} placeholder="Type a reply…" className="flex-1 bg-black/40 border rounded-lg px-3 py-2 text-sm" style={{ borderColor: C.border, color: C.text }} />
                      <button onClick={() => replyTo(t.id)} className="px-3 py-2 rounded-lg text-[11px] font-bold uppercase" style={{ background: C.cyan, color: '#06090F' }}>Reply</button>
                      <button onClick={() => resolve(t.id)} className="px-3 py-2 rounded-lg text-[11px] font-bold uppercase" style={{ background: C.green, color: '#06090F' }}>Resolve</button>
                    </div>
                  )}
                </div>
              )}
        ...
        <tbody>
          {logs.map((l) => (
            <tr key={l.id} className="border-t" style={{ borderColor: C.border }}>
              ...
              <td className="p-3 text-[11px]">{l.targetType ? `${l.targetType} #${l.targetId}` : '—'}</td>
              <td className="p-3 text-[11px]" style={{ color: C.sub }}>{l.details}</td>
              <td className="p-3 text-[11px] font-mono" style={{ color: C.sub }}>{when(l.createdAt)}</td>
            </tr>
          ))}
        </tbody>
      </table>
